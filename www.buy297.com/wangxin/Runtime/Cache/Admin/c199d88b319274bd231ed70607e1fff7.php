  ...
  <div class="page-content clearfix">
    ...
    <div class="search_style">
      ...
      <ul class="search_content clearfix">
         <form method="post" action="<?php echo U('Complain/hdlist');?>">
           <li><label class="l_f">活动名称</label><input name="name" type="text"  class="text_add" placeholder="输入服务名称"  style=" width:400px"/></li>
     ...
     <div class="border clearfix">
       <span class="l_f" style="margin-top:10px">
        <a href="<?php echo U('Complain/huodong');?>" id="member_add" class="btn btn-warning"><i class="icon-plus"></i>添加活动</a>
        <!-- <a href="<?php echo U('complain/excelfuwushang');?>" id="member_add" class="btn btn-warning">服务商信息导出</a> -->
    ...
    <thead>
     <tr>
        <th width="380">活动名称</th>
        <th width="280">活动排序</th>
        <th width="180">活动操作</th>
      </tr>
    </thead>
  <tbody>
    <?php if(is_array($con)): $i = 0; $__LIST__ = $con;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
     
          <td> <div style="border:1px solid black;border-radius:8px;width:180px;margin:0 auto;height:30px;text-align:center;line-height:30px;overflow:hidden"><a style="color:black" title="点击查看详情" href="<?php echo U('Complain/hdcon',array('id'=>$v['id']));?>"><?php echo ($v['title']); ?></a></div>
          </td>
          <td>
          <?php echo ($v['orders']); ?>
          </td>
          <td>
          <a href="<?php echo U('Complain/huodong',array('id'=>$v['id']));?>">更新</a>
          <a href="<?php echo U('Complain/delehuodong',array('id'=>$v['id']));?>" onclick="if(confirm('确认删除')){return true;}else{return false;}">删除</a>
          </td>
